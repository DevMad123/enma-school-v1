    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable('default_educational_settings')) {
            Schema::create('default_educational_settings', function (Blueprint $table) {
                ...
                $table->unique(['school_type', 'educational_level', 'setting_category', 'setting_key'], 'def_edu_set_uniq');
                $table->index(['school_type', 'setting_category'], 'idx_type_category');
            });
        }

        // Table d'audit des changements de configuration
        if (!Schema::hasTable('educational_settings_audit')) {
